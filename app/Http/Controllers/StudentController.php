...
class StudentController extends Controller
{
    ...
    function marksheet(Request $request)
    {
        ...
        $exams = Exam::from("exam as b")->join('exam_answer as a', 'a.testname', '=', 'b.name')->where('a.student_id', $sid)->where('b.publish', 'Yes')->selectRaw("exam_date,b.name,test_id,sum(mark)mark,(b.total_questions*4)total,markrange_file")->groupBy('test_id')->orderBy('b.exam_date', 'desc')->limit(5)->get()->map(function ($test) use ($batch, $type, $course) {
