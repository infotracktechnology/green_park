...
class StudentController extends Controller
{
    ...
    function marksheet(Request $request)
    {
        ...
        $exams = DB::table("exam_answer as a")->join('exam as b', 'a.test_id', '=', 'b.testid')->where('a.student_id', $sid)->where('b.publish', 'Yes')->selectRaw("exam_date,b.name,test_id,sum(mark)mark,(count(q_no)*4)total,markrange_file")->groupBy('test_id')->orderBy('b.updated_at', 'desc')->limit(5)->get()->map(function ($test) use ($batch) {
            return ['exam_date' => $test->exam_date, 'name' => $test->name, 'test_id' => $test->test_id, 'mark' => $test->mark, 'total' => $test->total,'first_mark' => ExamAnswer::where('test_id', $test->test_id)->selectRaw('SUM(mark) as mark')->groupBy('student_id')->orderByDesc('mark')->value('mark'),'markrange' => isset($test->markrange_file[$batch]) ? $test->markrange_file[$batch] : null];
        });
