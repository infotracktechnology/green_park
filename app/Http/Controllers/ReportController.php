...
class ReportController extends Controller
{
    public function section_exam(Request $request)
    {
        ...
        if ($request->query('type') == 'overall') {
            ...
            $answers = ExamAnswer::selectRaw("exam_answer.*,a.student_name")->join('student as a', 'exam_answer.student_id', '=', 'a.student_id')->where([['testname', $test_name], ['section', $section], ['coaching_type', 'OFFLINE']])->when(auth()->user()->branch, fn($q) => $q->where('a.campus', auth()->user()->branch))->orderBy('test_id')->orderBy('student_name')->get();
        ...
        if ($request->query('type') == 'omr') {
            ...
            $testIds = $answers->pluck('test_id')->unique();
            $key_correction = DB::table('key_corrections')->where('academic_year', $this->academic_year)->whereIn('testid', $testIds)->pluck('key_correction', 'testid');
   ...
   public function HostelAttendance(Request $request)
    ...
    ->get();
        ...
        foreach ($students as $student) {
            ...
            $rows = $rows->map(fn($row) => $this->normalizeRow($row));
    ...
    private function normalizeRow($row)
    {
        ...
        $baseCategory = $categorySubject
            ? trim(preg_replace('/\s*\([^)]*\)\s*$/', '', $category))
            : $category;
    ...
    private function buildDynamicReport($rows, $subjectMaxMarks = [], $section = null, $average = [])
    {
        ...
        foreach ($groups as $categoryName => $categoryRows) {
            ...
            foreach ($subjectLabels as $key => $label) {
                $maxQuestions = $categoryRows->max(function ($row) use ($key) {
                    $r = (int) ($row->{$key . '_r'} ?? 0);
                    $w = (int) ($row->{$key . '_w'} ?? 0);
                    $l = (int) ($row->{$key . '_l'} ?? 0);
...
$displayCategory = $categoryName;
    ...
    private function buildMergedReport($rows, $groupField = null, $includeOverallTop = false, $section = null, $isWeekend = false ) {
        ...
        foreach ($groups as $groupKey => $group) {
            ...
            foreach ($group as $examRow) {
                ...
                $examRecords = DB::table('examsubjectreport')
                    ->where('subject', $examSubject)
                    ->where('sec', $section)
                    ->get();
    ...
    private function allSubjectTotals($group)
    {
        ...
        $subjectKeys = $this->detectSubjectKeys($first);
        $subjects = [];
                    ...
                    if ($examSubject !== '') {
                        ...
                        foreach ($parts as $part) {
                            ...
                            $matches = (($part === 'PHY' || $part === 'PHYSICS') && $label === 'PHYSICS')
                                ...
                                || (($part === 'ZOO' || $part === 'ZOOLOGY') && $label === 'ZOOLOGY');
