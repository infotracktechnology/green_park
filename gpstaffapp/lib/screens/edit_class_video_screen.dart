...
class EditClassVideoScreen extends StatefulWidget {
  final dynamic videoId;

  const EditClassVideoScreen({super.key, required this.videoId});

  @override
  State<EditClassVideoScreen> createState() => _EditClassVideoScreenState();
}

class _EditClassVideoScreenState extends State<EditClassVideoScreen> {
  ...
  final List<String> _periods = ['1', '2', '3', '4', '5', '6'];
  ...
  Future<void> _fetchDetails() async {
    ...
    try {
      ...
      final res = await dio.get('/admin/classvideo/${widget.videoId}/edit');

      if (res.data != null && res.data['status'] == true) {
        final videoData = res.data['classvideo'];
        final model = ClassVideoModel.fromJson(videoData);

        _selectedSubject = model.subject ?? '';
        _chapterController.text = model.chapter ?? '';
        _selectedPeriod = model.period?.toString() ?? '';
  ...
  Future<void> _handleSubmit() async {
    ...
    if (_selectedPeriod.isEmpty) {
      _showErrorDialog('Validation Error', 'Please select a period.');
      return;
    }
    if (videoId.isEmpty) {
    ...
    try {
      ...
      final res = await dio.put(
        '/admin/classvideo/${widget.videoId}',
        data: data,
      );
        ...
        if (res.data != null && res.data['status'] == true) {
          showDialog(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: const Text('Class video updated successfully!'),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.pop(ctx);
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading || filters.loading) {
      return Scaffold(
        ...
        appBar: AppBar(title: const Text('Edit Class Video')),
        body: const Center(
          child: Column(
    ...
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
        title: const Text('Edit Class Video'),
      ),
