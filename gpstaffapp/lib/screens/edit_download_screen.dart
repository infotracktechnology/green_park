...
class EditDownloadScreen extends StatefulWidget {
  final dynamic downloadId;

  const EditDownloadScreen({super.key, required this.downloadId});

  @override
  State<EditDownloadScreen> createState() => _EditDownloadScreenState();
}

class _EditDownloadScreenState extends State<EditDownloadScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final res = await dio.get('/admin/download/${widget.downloadId}/edit');

      if (res.data != null && res.data['status'] == true) {
        final dlData = res.data['download'] as Map<String, dynamic>;
        final model = DownloadModel.fromJson(dlData);

        final Map<String, dynamic> combined = Map<String, dynamic>.from(dlData);
      ...
      debugPrint('Edit download fetch error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Failed to load download details'),
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      final res = await ApiClient().dio.post(
            '/admin/download/${widget.downloadId}',
            data: formData,
        ...
        if (res.data != null && res.data['status'] == true) {
          showDialog(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: const Text('Download updated successfully!'),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.pop(ctx);
  ...
  Widget build(BuildContext context) {
    ...
    if (_fetching || filters.loading) {
      return Scaffold(
        ...
        appBar: AppBar(title: const Text('Edit Download')),
        body: const Center(
          child: Column(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(_submitting ? 'Saving...' : 'Update Download',
