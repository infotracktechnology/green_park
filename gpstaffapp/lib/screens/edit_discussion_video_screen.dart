...
class EditDiscussionVideoScreen extends StatefulWidget {
  final dynamic videoId;

  const EditDiscussionVideoScreen({super.key, required this.videoId});

  @override
  State<EditDiscussionVideoScreen> createState() =>
      _EditDiscussionVideoScreenState();
}

class _EditDiscussionVideoScreenState extends State<EditDiscussionVideoScreen> {
  ...
  Future<void> _fetchDetails() async {
    ...
    try {
      ...
      final res =
          await dio.get('/admin/discussionvideo/${widget.videoId}/edit');

      if (res.data != null && res.data['status'] == true) {
        final videoData = res.data['discussionvideo'];
        final model = DiscussionVideoModel.fromJson(videoData);

        _dayController.text = model.day ?? '';
        if (model.date != null && model.date!.isNotEmpty) {
          try {
            _selectedDate = DateTime.parse(model.date!);
          } catch (_) {}
        }
        _selectedSubject = model.subject ?? '';
        _selectedPart = model.part ?? '';
  ...
  Future<void> _handleSubmit() async {
    ...
    try {
      ...
      final res = await dio.put(
        '/admin/discussionvideo/${widget.videoId}',
        data: data,
      );
        ...
        if (res.data != null && res.data['status'] == true) {
          showDialog(
            ...
            builder: (ctx) => AlertDialog(
              ...
              content: const Text('Discussion video updated successfully!'),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.pop(ctx);
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading || filters.loading) {
      return Scaffold(
        ...
        appBar: AppBar(title: const Text('Edit Discussion Video')),
        body: const Center(
          child: Column(
    ...
    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
        title: const Text('Edit Discussion Video'),
      ),
