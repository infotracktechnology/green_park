  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data.status) {
        ...
        Alert.alert('Error', res.data.message || 'Creation failed');
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center bg-slate-50">
        ...
        <Text className="text-xs text-gray-500 font-medium mt-3">Loading master data...</Text>
      </View>
    );
  }
        ...
        >
          {submitting ? (
            ...
            <>
              ...
              <Text className="text-white font-bold text-base">Publish Announcement</Text>
            </>
          )}
        </TouchableOpacity>
      </View>
