                ...
                <div class="col-12">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h4>Phone Turn Entry</h4>
                        </div>
                        
                        <div class="card-body">
                            <form method="GET" action="{{ route('phoneturn.create') }}">
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-2">
                                        <label>Amount</label>
                                        <input type="number" name="expense" class="form-control" value="{{ request('expense', 0) }}" min="0" required>
                                    </div>
                                    
                                    <div class="form-group col-lg-2">
                                        <label>Phone Date</label>
                                        <input type="date" name="phone_date" class="form-control" value="{{ request('phone_date', date('Y-m-d')) }}" required>
                                    </div>
                            ...
                            @if(request()->has('show'))
                                <form action="{{ route('phoneturn.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="branch_id" value="{{ request('branch_id') }}">
                                    <input type="hidden" name="hostel_id" value="{{ request('hostel_id') }}">
                                    <input type="hidden" name="room_no" value="{{ request('room_no') }}">
                                    <input type="hidden" name="phone_date" value="{{ request('phone_date') }}">
                                    <input type="hidden" name="expense" value="{{ request('expense') }}">
                                                        ...
                                                        <td>
                                                            <div class="custom-control custom-checkbox">
                                                                <input type="checkbox" name="student_id[]" value="{{ $student->student_id }}" 
                                                                    ...
                                                                    @checked($phoneTurnStudents->contains($student->student_id))>
...
<script>
    $(document).ready(function() {
        ...
        function fetchHostels(branchId, selectedId = null) {
            if (!branchId) return;
            hostelSelect.html('<option value="">Loading...</option>');
            
            $.get('{{ route("phoneturn.create") }}', { branch: branchId }, function(data) {
