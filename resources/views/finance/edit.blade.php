...
<div class="main-content">
   <section class="section">
          ...
          <div class="row">
              <div class="col-12">
                  <div class="card card-primary" x-data="feesplan()">
                     <form method="post" id="myForm" action="{{ route('feesplan.update', $feesplan->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                           ...
                           <div class="row">
                              ...
                              <div class="col-md-12 col-sm-12 mb-3 d-flex align-items-center justify-content-between">
                                 <h6 class="col-deep-purple">Edit Fees Plan</h6>
                           ...
                           <div class="row">
                                    ...
                                    <select name="coaching_type"  class="form-control form-control-sm select2" required>
                                      <option value="">Select  Coaching Type</option>
                                        @foreach ($coaching_type as $row)
                                            <option value="{{ $row->coaching_type }}" {{ $feesplan->coaching_type == $row->coaching_type ? 'selected' : '' }}>{{ $row->coaching_type }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group col-lg-4">
                                ...
                                <select name="branch_id" id="branch_id"  class="form-control form-control-sm select2 branch" required>
                                      <option value="">Select Branch</option>
                                        @foreach ($branchselect as $id => $branch)
                                            <option value="{{ $id }}" {{ $feesplan->branch_id == $id ? 'selected' : '' }}>{{ $branch }}</option>
                                        @endforeach
                                    </select>
                            </div>
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <select name="course" id="courseselect"  class="form-control form-control-sm select2 course" required>
                                      <option value="">Select Course</option>
                                        @foreach ($courseselect->where('campus', $feesplan->branch_id) as $course)
                                            <option value="{{ $course->course }}" {{ $feesplan->course == $course->course ? 'selected' : '' }}>{{ $course->course }}</option>
                                        @endforeach
                                    </select>
                            </div>
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <select name="batch[]" id="batchselect"  class="form-control form-control-sm select2 batch" required multiple>
                                      <option value="">Select Batch</option>
                                        @foreach ($batchselect->where('campus', $feesplan->branch_id)->where('course', $feesplan->course) as $batch)
                                            <option value="{{ $batch->batch }}" {{ in_array($batch->batch, explode(',', $feesplan->batch)) ? 'selected' : '' }}>{{ $batch->batch }}</option>
                                        @endforeach
                                    </select>
                            </div>
                                ...
                                <select name="bill_type_id" id="bill_type_id"  class="form-control form-control-sm select2" required>
                                      <option value="">Select Bill Type</option>
                                        @foreach ($bill_types as $id => $bill_type)
                                            <option value="{{ $id }}" {{ $feesplan->bill_type_id == $id ? 'selected' : '' }}>{{ $bill_type }}</option>
                                        @endforeach
                                    </select>
                            </div>
                              ...
                              <div class="form-group col-lg-4">
                                ...
                                <select name="is_hostel" id="hostel"  class="form-control form-control-sm select2" required>
                                      <option value="">Select Yes/No</option>
                                        <option value="1" {{ $feesplan->is_hostel == 1 ? 'selected' : '' }}>Yes</option>
                                        <option value="0" {{ $feesplan->is_hostel == 0 ? 'selected' : '' }}>No</option>
                                    </select>
                              </div>

                              <div class="form-group col-lg-4">
                                <label for="name">Fees Plan Name</label>
                                <input type="text" name="name" id="name" class="form-control form-control-sm" required value="{{ $feesplan->name }}">
                            </div>
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <select name="segment_id" id="segment"  class="form-control form-control-sm select2" required>
                                    ...
                                    @foreach ($segments->where('branch_id', $feesplan->branch_id) as $segment)
                                        <option value="{{ $segment->id }}" {{ $feesplan->segment_id == $segment->id ? 'selected' : '' }}>{{ $segment->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group col-lg-4">
                                ...
                                <select name="is_active" id="is_active"  class="form-control form-control-sm select2" required>
                                      <option value="">Select </option>
                                        <option value="1" {{ $feesplan->is_active == 1 ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ $feesplan->is_active == 0 ? 'selected' : '' }}>InActive</option>
                                    </select>
                              </div>
                           </div>
                           ...
                           <select x-model="feetype" x-on:change="removerows()" name="fee_type"  class="form-control form-control-sm" >
                            ...
                            <option value="{{ $row['feetype'] }}" {{ $feesplan->fee_type == $row['feetype'] ? 'selected' : '' }}>{{ $row['feetype'] }}</option>
                             @endforeach
                           </select>
                            </div>
    ...
    <tbody>
        ...
        {{-- Existing Rows --}}
        @foreach($feesplan->feePlanItems as $index => $item)
        <tr>
            <td>
                <input type="text"
                       name="item[{{ $index }}][instalment]"
                       class="form-control form-control-sm"
                       value="{{ $item->instalment }}" required>

                <input type="hidden"
                       name="item[{{ $index }}][id]"
                       value="{{ $item->id }}">
                <input type="hidden"
                       name="item[{{ $index }}][fee_type]"
                       value="{{ $item->fee_type }}">
            </td>
            ...
            <td>
                <input type="date"
                       name="item[{{ $index }}][invoice_date]"
                       class="form-control form-control-sm"
                       value="{{ $item->invoice_date }}" required>
            </td>

            <td>
                <input type="date"
                       name="item[{{ $index }}][due_date]"
                       class="form-control form-control-sm"
        ...
        {{-- Dynamic Rows --}}
        @php
            $counter = count($feesplan->feePlanItems); // starting index
        @endphp
            ...
            <tr>
                <td>
                    <input type="text"
                           class="form-control form-control-sm"
                           x-model="row.name"
                           :name="`item[${index + {{ $counter }} }][instalment]`"
                           required>
                    <input type="hidden" x-model="row.fee_type" :name="`item[${index + {{ $counter }} }][fee_type]`">
                </td>
                ...
                <td>
                    <input type="number"
                           class="form-control form-control-sm"
                           x-model="row.amount"
                           :name="`item[${index + {{ $counter }} }][amount]`"
                           required>
                </td>
                ...
                <td>
                    <input type="date"
                           class="form-control form-control-sm"
                           :name="`item[${index + {{ $counter }} }][invoice_date]`"
                           required>
                </td>
                ...
                <td>
                    <input type="date"
                           class="form-control form-control-sm"
                           :name="`item[${index + {{ $counter }} }][due_date]`"
                           required>
                </td>
            </tr>
        </template>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="form-group">
            <button type="button" class="btn btn-primary billsubmit">Update</button>
          </div>
